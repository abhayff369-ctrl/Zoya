export function processCommand(command: string): {
  ...
  // ========================================
  const contactNames = [
    "abhay", "rahul", "amit", "priya", "neha", "vikram",
    "sonam", "raj", "simran", "arjun", "kavya", "ananya",
    "mummy", "papa", "bhai", "behen", "friend"
  ];
  ...
  if (lowerCmd.includes("youtube")) {
    // "YouTube par video search karo [topic]"
    if (lowerCmd.match(/youtube\s+par\s+(?:video\s+)?search\s+karo\s+(.+)/)) {
      const query = lowerCmd.match(/youtube\s+par\s+(?:video\s+)?search\s+karo\s+(.+)/)?.[1] || "";
      return {
        action: `🎬 YouTube par "${query}" search kar raha hoon...`,
        appAction: "YOUTUBE_SEARCH",
    ...
    // "YouTube par [song] play karo"
    if (lowerCmd.match(/youtube\s+par\s+(.+?)\s+(?:play|bajao|chalao)/)) {
      const query = lowerCmd.match(/youtube\s+par\s+(.+?)\s+(?:play|bajao|chalao)/)?.[1] || "";
      return {
        action: `🎵 YouTube par "${query}" play kar raha hoon...`,
        appAction: "YOUTUBE_PLAY",
        appPackage: "com.google.android.youtube",
        intentData: { query },
        isBrowserAction: false,
        isAppAction: true,
    ...
    // "YouTube shorts dekho" / "YouTube reel dekho"
    if (lowerCmd.match(/youtube\s+(?:shorts|reel|reels|short)/)) {
      return {
        action: "📱 YouTube Shorts / Reels dekh raha hoon...",
        appAction: "YOUTUBE_SHORTS",
        appPackage: "com.google.android.youtube",
        isBrowserAction: false,
        isAppAction: true,
        isReelAction: true,
    ...
    // "YouTube trend dekho" / "Trending videos"
    if (lowerCmd.match(/youtube\s+(?:trend|trending|popular)/)) {
      return {
        action: "🔥 YouTube Trending videos dekh raha hoon...",
        appAction: "YOUTUBE_TRENDING",
        appPackage: "com.google.android.youtube",
        isBrowserAction: false,
        isAppAction: true,
      };
    ...
    // "YouTube par [channel] ka channel kholo"
    if (lowerCmd.match(/youtube\s+par\s+(\w+)\s+ka\s+channel/)) {
      const channel = lowerCmd.match(/youtube\s+par\s+(\w+)\s+ka\s+channel/)?.[1] || "";
      return {
        action: `📺 YouTube par ${channel} ka channel khol raha hoon...`,
        appAction: "YOUTUBE_CHANNEL",
        appPackage: "com.google.android.youtube",
        intentData: { channel },
        isBrowserAction: false,
        isAppAction: true,
    ...
    // "YouTube live dekho"
    if (lowerCmd.match(/youtube\s+(?:live|stream)/)) {
      return {
        action: "🔴 YouTube Live stream dekh raha hoon...",
        appAction: "YOUTUBE_LIVE",
        appPackage: "com.google.android.youtube",
        isBrowserAction: false,
        isAppAction: true,
      };
